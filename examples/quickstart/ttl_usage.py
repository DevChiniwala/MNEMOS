...
def basic_ttl_example():
    ...
    print(f"âœ… Created TTL stores with 30-day expiration")
...
def manual_cleanup_example():
    ...
    print("âœ… Created store with 1-minute TTL (auto-cleanup disabled)")
    ...
    # Wait a bit
    print("â³ Waiting 65 seconds for entries to expire...")
    time.sleep(65)
...
def ttl_disabled_example():
    ...
    print(f"âœ… Created store without TTL")
...
def flexible_ttl_example():
    ...
    for label, config in configs:
        ...
        print(f"   âœ“ {label:30s} = {ttl_days:6.2f} days")
...
def backward_compatibility_example():
    ...
    from mnemos import InMemoryMemoryStore
    ...
    print(f"   âœ… Successfully loaded {len(state.abstracts)} abstracts")
...
def main():
    """Run all examples"""
    print("=" * 60)
    print("Mnemos TTL (Time-To-Live) Examples")
    print("=" * 60)
    print()
    
    # Check API key
    if not os.getenv("OPENROUTER_API_KEY"):
        print("âš ï¸  OPENROUTER_API_KEY not set. Skipping full example.")
        ...
        print("\n" + "=" * 60)
        print("âœ… TTL Examples Complete!")
        print("=" * 60)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("âœ… All TTL Examples Complete!")
        print("=" * 60)
        print("\nKey Takeaways:")
        print("  â€¢ TTL stores automatically clean up old data")
        print("  â€¢ Auto-cleanup runs on load() by default")
        print("  â€¢ Manual cleanup available: cleanup_expired()")
        print("  â€¢ get_stats() shows total/valid/expired counts")
        print("  â€¢ TTL disabled (no params) = regular store behavior")
        print("  â€¢ Fully backward compatible with existing data")
