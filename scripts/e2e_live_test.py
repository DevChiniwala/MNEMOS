#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Live end-to-end test for Mnemos.
...
async def main() -> int:
    run_id = os.getenv("MNEMOS_E2E_RUN_ID") or str(uuid.uuid4())[:8]
    user_id = f"e2e_user_{run_id}"

    base_dir = Path(".tmp_e2e") / f"mnemos_{run_id}"
    if base_dir.exists():
    ...
    def _start_local_neo4j() -> tuple[str, str, str, str]:
        ...
        password = f"mnemos_e2e_{run_id}_pw"
        docker_neo4j_name = f"mnemos-neo4j-e2e-{run_id}"
        import subprocess
