...
Optional (defaults to localhost):
  ...
  NEO4J_PASSWORD=mnemos_local_password
...
async def main() -> int:
    ...
    from mnemos.profile import UserProfileStore, UserProfileAgent

    run_id = str(uuid.uuid4())[:8]
    user_id = f"stress_user_{run_id}"

    base_dir = Path(".tmp_e2e_stress") / f"mnemos_{run_id}"
    if base_dir.exists():
    ...
    neo4j_pass = _opt("NEO4J_PASSWORD", "mnemos_local_password")
    ...
    def make_agent():
        ...
        from mnemos.agents.research_agent import ResearchAgent
