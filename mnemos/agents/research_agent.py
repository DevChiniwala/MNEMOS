...
This module defines the ResearchAgent for the Mnemos (Mnemos) framework.
...
from mnemos.schemas import AdvancedMemoryStore, MemoryEntry
from mnemos.utils.checkpoint import CheckpointManager
from mnemos.learning.replay_buffer import ExperienceReplayBuffer
from mnemos.profile import UserProfileStore
try:
    from mnemos.graph import GraphMemoryStore, load_ontology_from_env
except ImportError:
    GraphMemoryStore = None  # type: ignore
    load_ontology_from_env = None  # type: ignore
try:
    from mnemos.retriever.graph_retriever import GraphRetriever
except ImportError:
        ...
        if hasattr(self, '_last_page_count') and current_page_count != self._last_page_count:
            ...
            for name, retriever in self.retrievers.items():
                try:
                    ...
                    print(f"âœ… Updated {name} retriever index")
                except Exception as e:
                    print(f"âŒ Failed to update {name} retriever: {e}")
