...
- `run.py`
  - CLI task dispatcher used by `mnemos-eval` console entrypoint.
  - routes a task name to a benchmark script in this folder.
- `ragas_eval.py`
  - package-level evaluator re-export (`mnemos.evaluation.ragas_eval`).
...
```bash
mnemos-eval <task> [task-specific-args...]
```
